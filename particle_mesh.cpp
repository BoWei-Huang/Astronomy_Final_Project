...
int index[n][3][2]={};//Define relevent index set
...
void test_potential()//test potential (正式拿掉)
{
	for (int i=0;i<N;i++){
        for (int j=0;j<N;j++){
            for (int k=0;k<N;k++){
...
//F_x        
    ...
    for (int i=0; i<n; i++){
        for (int j=0; j<2; j++){
            for (int k=0; k<2; k++){
                for (int p=0; p<2; p++){
                    particle_force[i][0]=particle_force[i][0]+num_force_x[index[i][0][j]][index[i][1][k]][index[i][2][p]]*(W(pos[i][0],x[index[i][0][j]])*W(pos[i][1],y[index[i][1][k]])*W(pos[i][2],z[index[i][2][p]]));
                    particle_force[i][1]=particle_force[i][1]+num_force_y[index[i][0][j]][index[i][1][k]][index[i][2][p]]*(W(pos[i][0],x[index[i][0][j]])*W(pos[i][1],y[index[i][1][k]])*W(pos[i][2],z[index[i][2][p]]));
                    particle_force[i][2]=particle_force[i][2]+num_force_z[index[i][0][j]][index[i][1][k]][index[i][2][p]]*(W(pos[i][0],x[index[i][0][j]])*W(pos[i][1],y[index[i][1][k]])*W(pos[i][2],z[index[i][2][p]]));    
...
//NGP method
if (method == 1) NGP_par_mesh();
//CIC method
if (method == 2) CIC_par_mesh();
...
//Testing Potential :Plummer's model (測試用，正式拿掉)
test_potential(); //測試用，正式拿掉
